...
function flexia_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'flexia' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		/* translators: %s: post author. */
		esc_html_x( 'by %s', 'post author', 'flexia' ),
...
function flexia_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list ) {
			...
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'flexia' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
		...
		if ( $tags_list ) {
			...
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'flexia' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
